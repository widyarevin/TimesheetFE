...
import { Navbar, Container, Table, Badge } from "react-bootstrap";
...
function Index() {
  ...
  const approve = (id) => {
    axios
      .put("http://localhost:8080/api/status/approveHr/" + id)
      .then((response) => {
        getData();
        console.log("Aprove successfully!");
      });
  };
  ...
  return (
    <>
      <Navbar bg="dark" variant="dark">
        <Container>
          <Navbar.Brand href="/">Home Human Resource</Navbar.Brand>
        </Container>
      </Navbar>
      ...
      <Container className="mt-4">
        ...
        <Table striped bordered hover size="sm">
          ...
          <tbody>
            {timesheet.map((item) => {
              if (item.status_name === "Approved PMO") {
                return (
                  <tr key={item.id}>
                    <td>{item.id}</td>
